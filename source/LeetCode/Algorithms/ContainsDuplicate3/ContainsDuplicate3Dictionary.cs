...
public sealed class ContainsDuplicate3Dictionary : IContainsDuplicate3
{
    ...
    public bool ContainsNearbyAlmostDuplicate(int[] nums, int indexDiff, int valueDiff)
    {
        ...
        for (var i = 0; i < nums.Length; i++)
        {
            ...
            if (numsDictionary.ContainsKey(m) || (numsDictionary.ContainsKey(m - 1) && Math.Abs(nums[i] - numsDictionary[m - 1]) < w) || (numsDictionary.ContainsKey(m + 1) && Math.Abs(nums[i] - numsDictionary[m + 1]) < w))
            {
                return true;
            }
