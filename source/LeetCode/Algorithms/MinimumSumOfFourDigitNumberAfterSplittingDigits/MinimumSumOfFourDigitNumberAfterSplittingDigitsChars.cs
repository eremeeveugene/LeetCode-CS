...
public sealed class MinimumSumOfFourDigitNumberAfterSplittingDigitsChars : IMinimumSumOfFourDigitNumberAfterSplittingDigits
{
    ...
    public int MinimumSum(int num)
    {
        ...
        return ((int)char.GetNumericValue(sortedNums[0]) * 10) + ((int)char.GetNumericValue(sortedNums[1]) * 10) + (int)char.GetNumericValue(sortedNums[2]) + (int)char.GetNumericValue(sortedNums[3]);
    }
}