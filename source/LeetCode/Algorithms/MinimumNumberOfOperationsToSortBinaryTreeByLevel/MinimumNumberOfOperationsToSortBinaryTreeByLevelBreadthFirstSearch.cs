...
public sealed class MinimumNumberOfOperationsToSortBinaryTreeByLevelBreadthFirstSearch : IMinimumNumberOfOperationsToSortBinaryTreeByLevel
{
    ...
    public int MinimumOperations(TreeNode root)
    {
        ...
        while (treeNodesQueue.Count > 0)
        {
            ...
            var indexDictionary = values.Select((value, index) => new { Value = value, Index = index }).OrderBy(x => x.Value)
                .Select((x, sortedIndex) => new { x.Index, SortedIndex = sortedIndex }).ToDictionary(x => x.Index, x => x.SortedIndex);
