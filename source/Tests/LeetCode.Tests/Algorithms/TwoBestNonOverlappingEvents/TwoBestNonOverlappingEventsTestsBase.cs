...
public abstract class TwoBestNonOverlappingEventsTestsBase<T> where T : ITwoBestNonOverlappingEvents, new()
{
    ...
    public void MaxTwoEvents_WithJaggedArrayOfEvents_ReturnsMaxSumOfTwoNonOverlappingEvents(string eventsJson,
        int expectedResult)
    {
        // Arrange
        var events = JsonHelper<int>.DeserializeToJaggedArray(eventsJson);
