...
public abstract class BoatsToSavePeopleTestsBase<T> where T : IBoatsToSavePeople, new()
{
    ...
    public void NumRescueBoats_GivenPeopleAndLimit_ReturnsExpectedNumberOfBoats(string peopleJson, int limit,
        int expectedResult)
    {
        // Arrange
        var people = JsonHelper<int>.DeserializeToArray(peopleJson);
        var solution = new T();
