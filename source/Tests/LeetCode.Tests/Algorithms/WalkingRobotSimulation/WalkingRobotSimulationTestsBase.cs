...
public abstract class WalkingRobotSimulationTestsBase<T> where T : IWalkingRobotSimulation, new()
{
    [TestMethod]
    [DataRow("[4,-1,3]", "[]", 25)]
    [DataRow("[4,-1,4,-2,4]", "[[2,4]]", 65)]
    [DataRow("[6,-1,-1,6]", "[]", 36)]
    [DataRow("[-2,8,3,7,-1]", "[[-4,-1],[1,-1],[1,4],[5,0],[4,5],[-2,-1],[2,-5],[5,1],[-3,-1],[5,-3]]", 324)]
    [DataRow("[9,-1,9,-2,3]", "[[4,8],[5,6],[7,7],[8,4]]", 225)]
    [DataRow("[1]", "[[2,2],[3,3]]", 1)]
    [DataRow("[9,-1,9,9,-2,9,9,-1,9,9,-2,9,9,9,-1,9,9,-2,9,9,-1,-1,9,9]", "[[10,10],[5,5],[8,8],[15,15],[20,20]]",
        8100)]
    public void RobotSim_WithMovementCommandsAndObstacles_ReturnsMaxSquaredDistanceFromOrigin(string commandsJson,
        string obstaclesJson, int expectedResult)
    {
        // Arrange
        var commands = JsonHelper.Parse<int[]>(commandsJson);
        var obstacles = JsonHelper.Parse<int[][]>(obstaclesJson);
