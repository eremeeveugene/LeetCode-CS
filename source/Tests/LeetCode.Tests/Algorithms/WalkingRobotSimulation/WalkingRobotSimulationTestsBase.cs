...
public abstract class WalkingRobotSimulationTestsBase<T> where T : IWalkingRobotSimulation, new()
{
    ...
    private static IEnumerable<object[]> GetTestData()
    {
        ...
        yield return [new[] { 5, -1, 5, -2, 5 }, Array.Empty<int[]>(), 50];

        yield return [new[] { 8, -1, 8, -1, 8 }, new[] { new[] { 4, 8 } }, 128];

        yield return [new[] { 3, 3, 3 }, Array.Empty<int[]>(), 36];

        yield return [new[] { 1, 1, 1, 1 }, new[] { new[] { 1, 1 } }, 4];
    }
}