...
public abstract class FindMinimumTimeToReachLastRoom1TestsBase<T> where T : IFindMinimumTimeToReachLastRoom1, new()
{
    ...
    public void MinTimeToReach_WithMoveTimeMatrix_ReturnsMinimumTimeRequired(int[][] moveTime, int expectedResult)
    {
        // Arrange
        var solution = new T();
    ...
    private static IEnumerable<object[]> GetTestData()
    {
        ...
        yield return [new[] { new[] { 0 }, new[] { 5 } }, 6];

        yield return [new[] { new[] { 0, 0, 0 } }, 2];

        yield return [new[] { new[] { 0, 10, 0 }, new[] { 0, 0, 0 } }, 3];

        yield return [new[] { new[] { 0, 0, 0 }, new[] { 0, 0, 0 }, new[] { 0, 0, 0 } }, 4];

        yield return [new[] { new[] { 0, 100 }, new[] { 0, 100 } }, 101];

        yield return [new[] { new[] { 0, 0, 0, 0 } }, 3];

        yield return [new[] { new[] { 0, 1, 2, 3 }, new[] { 0, 0, 0, 0 } }, 4];
    }
}